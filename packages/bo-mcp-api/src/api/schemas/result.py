...
class ResultBatchCreate(BaseModel):
    ...
    Results sharing parameter values with an existing result are
    replicates and are always accepted; repeated ingestion is prevented
    by experiment identity instead (suggestion ownership, one result per
    suggestion).
    """
...
class ResultSubmitResponse(ResponseEnvelope):
    ...
    :class:`bo_mcp_server.errors.ErrorCode` value when the operation
    failed, so REST clients can dispatch on the machine-readable code
    instead of string-matching ``errors`` — the same contract MCP
    clients get from the tool envelope's ``error.code``.
    """
