...
async def get_diagnostics(  # noqa: C901
    ...
    This tool provides comprehensive diagnostics for AI agents to assess
    optimization progress and make informed decisions.

    Args:
        campaign_id: UUID of the campaign
        use_cache: Whether to use cached results (default True, 30s TTL).
            Set to False to force fresh computation.
    ...
    async with get_session() as session:
        ...
        logger.info(
            "Diagnostics computed for campaign %s: results=%d, health=%s, converged=%s",
            campaign_id,
            len(results),
