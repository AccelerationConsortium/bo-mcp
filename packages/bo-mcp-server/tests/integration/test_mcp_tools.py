...
class TestCampaignLifecycle:
    """Tests for campaign lifecycle tools (pause, resume, terminate).

    Reference: State machine patterns for long-running operations
    """

    @pytest.mark.asyncio
    async def test_pause_campaign_invalid_id(self, setup_database):
        """Fails with invalid campaign_id format."""
        from bo_mcp_server.tools.campaign_lifecycle import pause_campaign

        result = await pause_campaign("not-a-uuid")

        assert result["success"] is False
        assert any("campaign_id" in e.lower() for e in result["errors"])

    @pytest.mark.asyncio
    async def test_pause_campaign_not_found(self, setup_database):
        """Fails when campaign does not exist."""
        from bo_mcp_server.tools.campaign_lifecycle import pause_campaign
    ...
    @pytest.mark.asyncio
    async def test_pause_campaign_from_created(self, setup_database):
        """Cannot pause campaign in CREATED status."""
        from bo_mcp_server.tools.campaign_lifecycle import pause_campaign
        from bo_mcp_server.tools.create_campaign import create_campaign
        ...
        result = await pause_campaign(create_result["campaign_id"])

        assert result["success"] is False
        assert any("cannot pause" in e.lower() for e in result["errors"])

    @pytest.mark.asyncio
    async def test_pause_running_campaign(self, setup_database):
        """Successfully pauses a running campaign."""
        from bo_mcp_server.tools.campaign_lifecycle import pause_campaign
        from bo_mcp_server.tools.create_campaign import create_campaign
        from bo_mcp_server.tools.generate_suggestions import generate_suggestions
    ...
    @pytest.mark.asyncio
    async def test_resume_campaign_from_paused(self, setup_database):
        """Successfully resumes a paused campaign."""
        ...
        create_result = await create_campaign(intake_data, owner_id)
        await generate_suggestions(create_result["campaign_id"])
        await pause_campaign(create_result["campaign_id"])

        result = await resume_campaign(create_result["campaign_id"])

        assert result["success"] is True
        assert result["status"] == "running"

    @pytest.mark.asyncio
    async def test_resume_non_paused_campaign(self, setup_database):
        """Cannot resume a non-paused campaign."""
        from bo_mcp_server.tools.campaign_lifecycle import resume_campaign
        from bo_mcp_server.tools.create_campaign import create_campaign
        from bo_mcp_server.tools.generate_suggestions import generate_suggestions

        owner_id = str(uuid4())
        intake_data = {
            "name": "Resume Non-Paused Test",
            "parameters": [{"name": "x", "type": "continuous", "bounds": [0, 1]}],
            "objectives": [{"name": "y", "direction": "minimize"}],
        }
        ...
        assert result["success"] is False
        assert any("cannot resume" in e.lower() for e in result["errors"])

    @pytest.mark.asyncio
    async def test_terminate_campaign(self, setup_database):
        """Successfully terminates a campaign."""
        ...
        result = await terminate_campaign(create_result["campaign_id"])

        assert result["success"] is True
        assert result["status"] == "completed"

    @pytest.mark.asyncio
    async def test_terminate_prevents_further_actions(self, setup_database):
        """Terminated campaign cannot be paused or resumed."""
        from bo_mcp_server.tools.campaign_lifecycle import (
            pause_campaign,
            resume_campaign,
            terminate_campaign,
        )
        ...
        owner_id = str(uuid4())
        intake_data = {
            "name": "Terminate Lock Test",
            "parameters": [{"name": "x", "type": "continuous", "bounds": [0, 1]}],
            "objectives": [{"name": "y", "direction": "minimize"}],
        }

        create_result = await create_campaign(intake_data, owner_id)
        await terminate_campaign(create_result["campaign_id"])

        pause_result = await pause_campaign(create_result["campaign_id"])
        resume_result = await resume_campaign(create_result["campaign_id"])
...
class TestSearchTools:
    """Tests for search_tools meta-tool.
    ...
    async def test_search_by_keyword(self):
        """Finds tools matching keyword search."""
        from bo_mcp_server.tools.search_tools import search_tools

        result = await search_tools(query="create campaign")

        assert result["success"] is True
        assert len(result["results"]) > 0
        # Should find create_campaign as most relevant
        tool_names = [r["name"] for r in result["results"]]
        assert "create_campaign" in tool_names

    @pytest.mark.asyncio
    async def test_search_by_category(self):
        """Finds all tools in a category."""
        from bo_mcp_server.tools.search_tools import search_tools

        result = await search_tools(query="", category="optimization")

        assert result["success"] is True
        # Should include generate_suggestions and submit_results
        tool_names = [r["name"] for r in result["results"]]
        assert "generate_suggestions" in tool_names
        assert "submit_results" in tool_names

    @pytest.mark.asyncio
    async def test_search_invalid_category(self):
        """Invalid category returns error."""
        from bo_mcp_server.tools.search_tools import search_tools

        result = await search_tools(query="test", category="nonexistent")

        assert result["success"] is False
        assert any("invalid" in e.lower() for e in result["errors"])

    @pytest.mark.asyncio
    async def test_search_returns_categories(self):
        """Search result includes available categories."""
        from bo_mcp_server.tools.search_tools import search_tools

        result = await search_tools(query="")
