...
class TestSuggestionReproducibility:
    ...
    async def test_bo_suggestions_deterministic_with_seed(self, setup_database):
        ...
        With fixed seed and same data, GP model fitting and acquisition
        optimization should produce identical results.
        """
        from bo_mcp_server.tools.create_campaign import create_campaign
        from bo_mcp_server.tools.generate_suggestions import generate_suggestions
        ...
        # Fixed observation data
        fixed_results = [
            {"parameter_values": {"x": 0.2}, "objective_values": {"f1": 0.5, "f2": 0.8}},
            {"parameter_values": {"x": 0.5}, "objective_values": {"f1": 0.6, "f2": 0.6}},
            {"parameter_values": {"x": 0.8}, "objective_values": {"f1": 0.8, "f2": 0.5}},
        ]
        ...
        create1 = await create_campaign(intake_data, owner_id)
        campaign1_id = create1["campaign_id"]
        await generate_suggestions(campaign1_id)  # Initial design
        await submit_results(campaign1_id, fixed_results, owner_id)

        torch.manual_seed(42)  # Reset seed
        gen1_bo = await generate_suggestions(campaign1_id)
        ...
        create2 = await create_campaign(intake_data, owner_id)
        campaign2_id = create2["campaign_id"]
        await generate_suggestions(campaign2_id)  # Initial design
        await submit_results(campaign2_id, fixed_results, owner_id)

        torch.manual_seed(42)  # Reset seed
        gen2_bo = await generate_suggestions(campaign2_id)

        # BO suggestions should be very close (may have minor numerical differences)
        assert gen1_bo["success"] is True
        assert gen2_bo["success"] is True

        # Check that suggestions are generated via BO
        for s in gen1_bo["suggestions"]:
            assert s["provenance"]["generation_method"] == "bo"

        # Due to potential numerical instabilities, use tolerance
        for s1, s2 in zip(gen1_bo["suggestions"], gen2_bo["suggestions"], strict=True):
            x1 = s1["parameter_values"]["x"]
            x2 = s2["parameter_values"]["x"]
            # Allow small tolerance for numerical differences
            assert abs(x1 - x2) < 0.05, f"BO suggestions differ: {x1} vs {x2}"
